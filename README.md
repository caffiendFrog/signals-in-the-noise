...
2. Create and activate the conda environment
   ...
   ```
   This installs all dependencies **and** the project package in editable mode (`-e .`),
   so every notebook can `import signals_in_the_noise` immediately.

3. Download the datasets
   ```bash
   python bin/download_datasets.py
   ```
   The project uses [GSE161529](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE161529).
   Datasets are too large for GitHub and are downloaded directly from the Gene Expression
   Omnibus (GEO). The script places files under `data/` in the layout expected by all notebooks.

   > "Processed scRNA-seq and bulk RNA-seq data generated for this study are available as
   > GEO series GSE161529 and GSE161892, respectively. Raw data are available on request,
   > subject to approval by our institutional Data Access Committee
   > (dataaccess@wehi.edu.au) to ensure preservation of patient confidentiality."

4. Launch JupyterLab
   ```bash
   jupyter lab
   ```
   Open any notebook under `notebooks/GSE161529/` to begin.
...
| Directory | Purpose |
|---|---|
| `bin/` | Scripts to download datasets |
| `data/` | Data files — gitignored due to size |
| `images/` | Images used in documentation |
| `notebooks/` | Jupyter notebooks (primary analysis in `GSE161529/`, scratch work in `scratch/`) |
| `resources/` | Reference PDFs, gene signature files, GSEA gene sets |
| `src/` | Reusable Python package (`signals_in_the_noise`) |
| `tests/` | pytest unit tests mirroring `src/` layout |
